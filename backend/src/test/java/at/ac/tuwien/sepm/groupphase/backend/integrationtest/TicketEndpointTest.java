...
import at.ac.tuwien.sepm.groupphase.backend.endpoint.PerformanceEndpoint;
...
public class TicketEndpointTest implements TestData {
    ...
    @Autowired
    private TicketPdfRepository ticketPdfRepository;
    @Autowired
    ...
    @Test
    public void givenTicketWithPdf_whenRequestTicketBelongingToThisUser_thenGetPdf() throws Exception {
        ...
        String pdf = this.ticketService.createPdfOfTicket(ticket1.getId());
        TicketPdf ticketPdf = TicketPdf.builder()
            .ticketPdf(pdf)
            .build();
        this.ticketPdfRepository.save(ticketPdf);
        this.ticket.setTicketPdf(ticketPdf);
        this.ticket = this.ticketRepository.save(this.ticket);


        MvcResult mvcResult = this.mockMvc.perform(get(TICKET_BASE_URI + "/pdf/" + this.ticket.getId())
        ...
        TicketPdfDto ticketPdfDto = objectMapper.readValue(response.getContentAsString(), TicketPdfDto.class);
        assertThat(ticketPdfDto.getPdf()).isNotNull();
        assertThat(ticketPdfDto.getPdf()).isBase64();
    }

    @Test
    public void givenOrder_whenRequestingTicketOfOtherUserAsAdmin_thenGetPdf() throws Exception {
        this.location = this.locationRepository.save(this.location);
        this.eventHall.setLocation(this.location);
        this.eventHall = this.eventHallRepository.save(this.eventHall);
        ...
        this.otherUser.setAdmin(true);
        this.userRepository.save(this.otherUser);


        MvcResult mvcResult = this.mockMvc.perform(get(TICKET_BASE_URI + "/pdf/" + this.ticket.getId())
                .header(securityProperties.getAuthHeader(), jwtTokenizer.getAuthToken(otherUser.getUsername(), ADMIN_ROLES)))
            .andDo(print())
            .andReturn();
    ...
    @Test
    public void givenOrder_whenRequestingTicketOfOtherUserAsNormalUser_thenGet403() throws Exception {
        this.userRepository.save(this.user);
        this.userRepository.save(this.otherUser);
        this.ticket.setApplicationUser(this.user);
        Ticket ticket1 = this.ticketRepository.save(this.ticket);

        MvcResult mvcResult = this.mockMvc.perform(get(TICKET_BASE_URI + "/pdf/" + ticket1.getId())
    ...
    @Test
    public void givenNothing_whenRequestingTicketThatDoesNotBelongToAnyone_thenGet403() throws Exception {
        this.userRepository.save(this.user);
        this.userRepository.save(this.otherUser);
    ...
    @Test
    public void givenNothing_whenSearchingForNonExistingTicket_thenGet404() throws Exception {
        this.userRepository.save(this.user);

        MvcResult mvcResult = this.mockMvc.perform(get(TICKET_BASE_URI + "/pdf/" + (-23))
                .header(securityProperties.getAuthHeader(), jwtTokenizer.getAuthToken(otherUser.getUsername(), USER_ROLES)))
            .andDo(print())
            .andReturn();
